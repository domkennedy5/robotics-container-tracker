...
# ── tabs ───────────────────────────────────────────────────────────────────────
st.title("Robotics Container Tracker")
tab1, tab2, tab3, tab4, tab5, tab6, tab7, tab8 = st.tabs(["Container Lookup", "Carrier Submission", "Empty Returns", "Carrier Data", "Lane Costs", "Insights", "Planning", "Help"])
...
with tab8:
    ...
    st.caption("Quick-reference guide — find what you need in under a minute.")

    # ── What is this tool ─────────────────────────────────────────────────────
    st.markdown("""
**Robotics Container Tracker** is an AGL operations tool for managing inbound robotics container
deliveries. It covers two main workflows:

| Workflow | What you need | Where to go |
|---|---|---|
| Build and manage this week's delivery plan | Nothing — standalone | **Planning** tab |
| Look up containers, track empties, view risk | Weekly DBR file (sidebar) | All other tabs |
""")
    ...
    st.markdown("""
Do these steps every Monday before you start planning.

**Step 1 — Refresh historical data (2 min)**
1. Download `ToteASERs Robotics DBR Tracker.xlsx` from SharePoint
2. Go to **Planning → Import History**
3. Upload the file → click **Import**
4. New containers since last week are added automatically; duplicates are skipped

**Step 2 — Load the weekly DBR (sidebar)**
1. Open the sidebar (top-left arrow)
2. Upload the current week's DBR Excel file under **DBR File**
3. This activates: Container Lookup, Empty Returns, Insights, and the DBR Snapshot

**Step 3 — Build the week's delivery plan**
1. Go to **Planning → Plan Builder**
2. Select the week using the week selector at the top
3. Paste Miguel's or site ops' message into **Step 1 — Parse Stakeholder Request** → click Parse
4. Add containers individually (Single tab) or in bulk (Bulk Paste tab)
5. Review the week grid at the bottom — it shows daily load by carrier

**Step 4 — Send daily notifications**
1. Go to **Planning → By Site**
2. Select the site
3. Open **Daily Notification — Copy for Slack**
4. Pick the delivery date → copy the text → paste into Slack

**Step 5 — Export for carriers**
1. Go to **Planning → Carrier View**
2. Select a carrier
3. Click **Export** — generates a clean Excel with no internal status notes
""")
    ...
    with st.expander("Container Lookup", expanded=False):
        st.markdown("""
**What it does:** Cross-references container IDs against the loaded DBR to show status,
terminal, FC destination, SLA flags, and more.

**To use:**
1. Upload the DBR in the sidebar first (required)
2. Paste any list of container IDs into the text box — dashes and check digits optional
3. Click **Search**
4. Results show status across Delivery Appointments, Empty Returns, On Vessel, Demurrage, and Accessorials
5. Download results as Excel with the export button

**Tip:** You can paste a raw email list — the tool normalizes IDs automatically.
""")

    with st.expander("Carrier Submission", expanded=False):
        st.markdown("""
**What it does:** Accepts carrier status updates via template upload or manual entry,
and logs them to the database.

**To use (template upload — preferred):**
1. Download the **AGL Carrier Template** using the button at the top right
2. Give it to your carrier to fill out
3. Enter the carrier name, upload the completed file, click **Confirm & Submit All**

**To use (manual entry):**
1. Fill in the form at the bottom of the tab
2. Paste container IDs or upload a carrier status file
3. Click **Submit**

**Submission Log** at the bottom shows all historical entries, filterable by carrier, status, and source.
""")

    with st.expander("Empty Returns", expanded=False):
        st.markdown("""
**What it does:** Shows all containers with outstanding empty return obligations,
sorted by urgency.

**To use:**
1. DBR must be loaded in the sidebar
2. Use the radio buttons to filter: **Overdue**, **Due Soon (≤3 days)**, **All Active**, or **All incl. Terminated**
3. The table sorts by days remaining — red flags appear for overdue containers

**Key columns:** Container #, Terminal, Empty Return Due Date, Days Until Due, Alert status
""")

    with st.expander("Carrier Data", expanded=False):
        st.markdown("""
**What it does:** Full view of everything submitted by carriers — filterable by carrier,
sheet type, and SLA status.

**To use:**
1. Filter by carrier, sheet type (Delivery, Empty Returns, ODY, Demurrage, Accessorials), or SLA
2. Sub-tabs organize by sheet type automatically
3. Export everything to Excel with one button

**Note:** This tab only has data after carriers have submitted via the Carrier Submission tab.
""")

    with st.expander("Lane Costs", expanded=False):
        st.markdown("""
**What it does:** Rate matrix and scenario cost builder by port → node → carrier.

**To use:**
1. Filter by port, rate source, and lane type at the top
2. **Rate Matrix** shows cheapest vs. most expensive carrier per lane (color-coded)
3. **Scenario Builder** — select port, node, carrier, enter container count → click **Add Lane**
4. Build a multi-lane scenario; the tool calculates total cost and compares against cheapest available

**Note:** Requires rate data to be loaded by the app admin. Shows an info message if empty.
""")

    with st.expander("Insights", expanded=False):
        st.markdown("""
**What it does:** Auto-generated weekly summary across all data sources.

**Three sections — each activates as its source data is uploaded:**

| Section | Requires | Shows |
|---|---|---|
| DBR Snapshot | DBR in sidebar | Delivery count, empty return risk, LFD risk, status/terminal breakdown |
| Detention & Demurrage Risk | Inbound Loads Report (sidebar) | Containers within 7 days of free-time expiry |
| In-Transit Pipeline | Import Shipment Status (sidebar) | Active containers by status, vessel, FC |

**To load supplemental reports:** Open the sidebar → upload under **Additional Reports**.
""")

    with st.expander("Planning", expanded=False):
        st.markdown("""
**What it does:** Full delivery plan builder — create, manage, and export the weekly
container schedule across all sites and carriers. Standalone — no DBR upload required.

**Sub-tabs:**

| Sub-tab | Use it to |
|---|---|
| Plan Builder | Parse stakeholder requests, add containers (single or bulk), view the week grid |
| All Sites | Full compiled view of the week across all sites; export to Excel |
| By Site | Site-specific view, daily notification for Slack, inline edit |
| Carrier View | Carrier-specific view, shareable Excel export (no internal status) |
...
**Receiving days:** The day selector at the top of Planning lets you toggle which days are active
for that week (Mon–Fri default). Changing active days will offer to redistribute unassigned
...
| I want to... | Go to |
|---|---|
| Find a container's status | Container Lookup — paste ID, click Search |
| See what's overdue on empty returns | Empty Returns tab |
...
| See all containers at risk of detention fees | Insights → Detention & Demurrage Risk |
| Load new carrier data | Sidebar → upload DBR or Additional Reports |
| Add a new site or carrier | Planning → Config |
| Load historical data | Planning → Import History |
""")

    st.divider()
    st.caption("Password: robotics2026  ·  Questions? Contact Dominique Kennedy (AGL)")
